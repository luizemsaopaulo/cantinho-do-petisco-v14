(() => {
  ...
  function renderAddButton(){const p=state.selected;if(!p)return;const price=unitPriceForSelected(),needsChoice=!selectionComplete(),disabled=!p.available||price==null;if(!needsChoice&&price!=null)els.itemPrice.textContent=money.format(Number(price));els.addItem.disabled=disabled;els.addItem.setAttribute('aria-invalid',needsChoice?'true':'false');els.addItem.textContent=!p.available||price==null?'Indisponível':needsChoice?'Adicionar ao carrinho':`Adicionar • ${money.format(Number(price)*state.selectedQty)}`;}
