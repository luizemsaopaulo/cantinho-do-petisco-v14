(() => {
  ...
  const state={allCategories:[],categories:[],products:[],specials:[],optionGroups:[],productOptions:[],cart:[],query:'',category:'all',selected:null,selectedQty:1,selectedVariants:[],selectedOptions:new Map(),selectedOptionQty:new Map(),specialOverride:null,groupMap:new Map()};
  ...
  function resetSelectedOptions(){state.selectedOptions=new Map();state.selectedOptionQty=new Map();}
  ...
  function selectionComplete(){
    const p=state.selected;if(!p)return false;
    return optionGroupsForProduct(p.id).every(g=>{const n=selectedIdsForGroup(g.id).size,min=groupMin(g),max=groupMax(g);return n>=min&&(max==null||n<=max);});
  }
  ...
  function handleOptionInput(input){
    ...
    state.selectedOptions.set(g.id,ids);renderOptionGroups();renderAddButton();
  }
  function changeOptionQty(groupId,optionId,delta){
    ...
    if(next<=0){ids.delete(o.id);state.selectedOptionQty.delete(o.id);state.selectedOptions.set(g.id,ids);renderOptionGroups();renderAddButton();return;}
    ...
    state.selectedOptionQty.set(o.id,next);state.selectedOptions.set(g.id,ids);renderOptionGroups();renderAddButton();
  }
  function renderOptionGroups(){
    const p=state.selected;if(!p||!els.itemOptions)return;const groups=optionGroupsForProduct(p.id);
    ...
    els.itemOptions.innerHTML=groups.map(g=>{const opts=optionsForGroup(g.id),ids=selectedIdsForGroup(g.id),type=g.selection_type==='single'?'radio':'checkbox';return`<fieldset class="item-option-group" data-option-group-block="${escapeHtml(g.id)}"><legend><span>${escapeHtml(g.name)}</span>${g.required?'<b>Obrigatório</b>':'<small>Opcional</small>'}</legend><p class="option-group-hint">${escapeHtml(optionGroupHint(g))}</p><div class="option-choice-list">${opts.map(o=>{const qtyEnabled=optionAllowsQuantity(o,g),qty=ids.has(o.id)?selectedOptionQuantity(o,g):0,maxQty=maxOptionQuantity(o);if(qtyEnabled)return`<div class="option-choice option-choice-quantity ${qty>0?'selected':''}"><span class="option-choice-copy"><strong>${escapeHtml(o.name)}</strong><small>${escapeHtml(optionPriceText(o))} cada${maxQty!=null?` · máx. ${maxQty}`:' · quantidade livre'}</small></span><div class="option-inline-qty" aria-label="Quantidade de ${escapeHtml(o.name)}"><button type="button" data-option-qty-dec="${escapeHtml(o.id)}" data-option-qty-group="${escapeHtml(g.id)}" ${qty<=0?'disabled':''}>−</button><strong>${qty}</strong><button type="button" data-option-qty-inc="${escapeHtml(o.id)}" data-option-qty-group="${escapeHtml(g.id)}" ${maxQty!=null&&qty>=maxQty?'disabled':''}>+</button></div></div>`;return`<label class="option-choice ${ids.has(o.id)?'selected':''}"><input type="${type}" name="option-${escapeHtml(g.id)}" value="${escapeHtml(o.id)}" data-option-group="${escapeHtml(g.id)}" ${ids.has(o.id)?'checked':''}><span class="option-choice-copy"><strong>${escapeHtml(o.name)}</strong><small>${escapeHtml(optionPriceText(o))}</small></span><span class="option-choice-mark" aria-hidden="true">${type==='radio'?'●':'✓'}</span></label>`;}).join('')}</div></fieldset>`;}).join('');
  ...
  function addSelected(){const p=state.selected;if(!p)return;if(!selectionComplete()){toast('Escolha as opções obrigatórias antes de adicionar.','error');return;}const price=unitPriceForSelected();if(price==null||!p.available)return;const note=p.allow_notes===false?'':els.itemNote.value.trim(),options=selectedOptionRows(),optionKey=options.map(o=>`${o.option_id}:${Math.max(1,Number(o.quantity)||1)}`).sort().join(','),key=`${p.id}|${optionKey}|${note}|${price}`,existing=state.cart.find(x=>x.key===key);if(existing)existing.qty=Math.min(99,existing.qty+state.selectedQty);else state.cart.push({key,product_id:p.id,name:p.name,size:p.size||'',price:Number(price),qty:state.selectedQty,note,options});saveCart();renderCart();els.mobileCartBar.classList.remove('bump');requestAnimationFrame(()=>{els.mobileCartBar.classList.add('bump');setTimeout(()=>els.mobileCartBar.classList.remove('bump'),420);});toast(`${p.name}${p.size?` · ${p.size}`:''} adicionado ao pedido.`,'success');}
